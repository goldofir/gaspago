...
import { QrCode, Clock, Settings, Flame, Menu, X, ChevronRight, LogOut } from 'lucide-react'
...
export default function PosLayout({ children }: { children: ReactNode }) {
  ...
  const isLoginPage = path === '/pos/login'

  useEffect(() => {
    if (isLoginPage) { setChecked(true); return }
    const token = localStorage.getItem('gp_pos_token')
    if (!token) {
  ...
  function handleLogout() {
    localStorage.removeItem('gp_pos_token')
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--ground)' }}>
      <style>{`
        ...
        .pos-main   { margin-left: 248px; flex: 1; padding: 36px 40px; min-height: 100vh; }
        .pos-overlay { display: none; }
        @media (max-width: 767px) {
          .pos-sidebar { transform: translateX(-100%); width: 280px; }
          ...
          .pos-main  { margin-left: 0; padding: 72px 16px 32px; }
          .pos-overlay { display: block; position: fixed; inset: 0; background: rgba(0,0,0,.5); z-index: 48; opacity: 0; pointer-events: none; transition: opacity .25s; }
          .pos-overlay.open { opacity: 1; pointer-events: auto; }
        }
      `}</style>

      <div className={`pos-overlay${open ? ' open' : ''}`} onClick={() => setOpen(false)} />
      ...
      <aside className={`pos-sidebar${open ? ' open' : ''}`}>
        <div style={{ padding: '22px 20px 18px' }}>
          ...
          <div style={{ marginTop: 10, padding: '4px 8px', borderRadius: 5, background: 'rgba(16,185,129,.12)', border: '1px solid rgba(16,185,129,.2)', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
            <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#10B981' }} />
            <span style={{ fontSize: 10.5, fontWeight: 600, color: '#10B981', letterSpacing: '.08em', textTransform: 'uppercase' }}>Estabelecimento</span>
          </div>
        </div>
        <div style={{ height: 1, background: 'rgba(255,255,255,.06)', margin: '0 16px' }} />
        <nav style={{ flex: 1, padding: '10px 10px', overflowY: 'auto' }}>
          <p style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,.25)', letterSpacing: '.1em', textTransform: 'uppercase', padding: '8px 10px 6px' }}>Balcão</p>
          {nav.map(({ href, label, Icon }) => {
            const exact = href === '/pos'
            ...
            return (
              <Link key={href} href={href} onClick={() => setOpen(false)} style={{
                ...
                color: active ? '#fff' : 'rgba(255,255,255,.55)', background: active ? 'rgba(16,185,129,.14)' : 'transparent',
