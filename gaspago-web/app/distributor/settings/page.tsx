...
import { Building2, Phone, Mail, MapPin, FileText, Bell, LogOut } from 'lucide-react'
...
function InfoRow({ Icon, label, value }: { Icon: any; label: string; value: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
      <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(59,130,246,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
export default function DistributorSettingsPage() {
  ...
  useEffect(() => {
    portalFetch(TOKEN_KEY, `${API}/distributors/me`)
      .then(r => r.json())
      .then(setDist)
  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '20px 22px', marginBottom: 24, boxShadow: 'var(--shadow-sm)' }}>
        <h2 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 6 }}>Dados da distribuidora</h2>
        {loading ? (
          ...
          <div>
            <InfoRow Icon={Building2} label="Razão social" value={dist.name} />
            <InfoRow Icon={FileText} label="CNPJ" value={dist.cnpj} />
            <InfoRow Icon={Phone} label="Telefone" value={dist.phone} />
            <InfoRow Icon={Mail} label="E-mail" value={dist.email ?? ''} />
            <div style={{ padding: '12px 0 0' }}>
              <InfoRow Icon={MapPin} label="Endereço" value={`${dist.address}, ${dist.city} - ${dist.state}, ${dist.postalCode}`} />
            </div>
          </div>
        )}
      </div>
      ...
      <AsaasDocumentSection tokenKey={TOKEN_KEY} endpointPrefix="/distributors/me" />
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '20px 22px', marginBottom: 24, boxShadow: 'var(--shadow-sm)' }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'not-allowed' }} title="Preferência de notificação ainda não é salva no servidor — em breve">
          ...
          <span style={{ fontSize: 13.5, color: 'var(--text)', fontWeight: 500, flex: 1 }}>Receber notificações de novos pedidos (em breve)</span>
          <input
            type="checkbox"
    ...
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '20px 22px', marginBottom: 24, boxShadow: 'var(--shadow-sm)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--muted)' }}>Envie a documentação da empresa para liberar saques e transferências de saldo na produção.</p>
        </div>
        <div style={{
