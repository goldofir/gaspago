...
import { LayoutDashboard, ShoppingBag, Package, Settings, Flame, Menu, X, ChevronRight } from 'lucide-react'
...
export default function DistributorLayout({ children }: { children: ReactNode }) {
  ...
  const isLoginPage = path === '/distributor/login'

  useEffect(() => {
    if (isLoginPage) { setChecked(true); return }
    const token = localStorage.getItem('gp_distributor_token')
    if (!token) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--ground)' }}>
      <style>{`
        ...
        .dist-topbar { display: none; }
        .dist-main   { margin-left: 248px; flex: 1; padding: 36px 40px; min-height: 100vh; }
        .dist-overlay { display: none; }
        @media (max-width: 767px) {
          .dist-sidebar { transform: translateX(-100%); box-shadow: 4px 0 32px rgba(0,0,0,.35); width: 280px; }
          .dist-sidebar.open { transform: translateX(0); }
          .dist-topbar {
          ...
          .dist-main  { margin-left: 0; padding: 72px 16px 32px; }
          .dist-overlay {
            display: block; position: fixed; inset: 0; background: rgba(0,0,0,.5);
            backdrop-filter: blur(2px); z-index: 48; opacity: 0;
            pointer-events: none; transition: opacity .25s;
          }
          .dist-overlay.open { opacity: 1; pointer-events: auto; }
        }
      `}</style>

      <div className={`dist-overlay${open ? ' open' : ''}`} onClick={() => setOpen(false)} />
      ...
      <aside className={`dist-sidebar${open ? ' open' : ''}`}>
        <div style={{ padding: '22px 20px 18px' }}>
          ...
          <div style={{ marginTop: 10, padding: '4px 8px', borderRadius: 5, background: 'rgba(59,130,246,.12)', border: '1px solid rgba(59,130,246,.2)', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
            <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#3B82F6' }} />
            <span style={{ fontSize: 10.5, fontWeight: 600, color: '#3B82F6', letterSpacing: '.08em', textTransform: 'uppercase' }}>Distribuidora</span>
          </div>
        </div>
          ...
          {nav.map(({ href, label, Icon }) => {
            ...
            return (
              <Link key={href} href={href} onClick={() => setOpen(false)} style={{
                ...
                color: active ? '#fff' : 'rgba(255,255,255,.55)', background: active ? 'rgba(59,130,246,.14)' : 'transparent',
      ...
      <main className="dist-main">{children}</main>
    </div>
  )
}
