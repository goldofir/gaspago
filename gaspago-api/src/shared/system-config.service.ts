...
export const KNOWN_KEYS: Record<string, { label: string; group: string; sensitive: boolean; hint?: string; readonly?: boolean }> = {
  ...
  DIRECT_REFERRER_PCT: { label: 'Comissão do indicador direto (%)', group: 'commissions', sensitive: false, hint: 'Vai pra quem realmente indicou o comprador (não pra quem está acima dele na matriz — pode ser gente diferente por causa de transbordamento). Paga além da rede por nível abaixo, não em vez dela. Só usado hoje na compra de plano — veja também Planos, onde dá pra sobrescrever por plano.' },
  MATRIX_LEVEL_1_PCT: { label: 'Rede — Nível 1 (%)', group: 'commissions', sensitive: false, hint: 'Comissão de rede por posição na matriz (1 nível de matriz acima de quem comprou — pode não ser quem indicou de verdade, veja "Comissão do indicador direto" acima).' },
